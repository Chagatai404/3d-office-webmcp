import { NextResponse } from "next/server";
import { requestJoinByPasscode } from "@/domain/rooms/operations";
import { actionResponse, authenticateRoomRequest } from "@/app/api/_shared/request";
import { consumeJoinAttempt, joinRateLimitResponse } from "@/app/api/_shared/join-rate-limit";

export async function POST(request: Request) {
  const auth = await authenticateRoomRequest(request);
  if (!auth) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const body = await request.json().catch(() => null);
  const target = typeof body === "object" && body !== null && "roomId" in body
    ? String(body.roomId)
    : "invalid-passcode-request";
  const retryAfter = consumeJoinAttempt({ request, actorUserId: auth.userId, target: `passcode:${target}` });
  if (retryAfter !== null) return joinRateLimitResponse(retryAfter);
  return actionResponse(await requestJoinByPasscode(auth.repository, body, {
    authUserId: auth.userId, origin: "manual_ui",
  }));
}
