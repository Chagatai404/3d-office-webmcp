-- propose_participant_tradeoff (backing the WebMCP respond_to_concern tool)
-- required every referenced conflict's proposal_id to equal the room's
-- CURRENT active_proposal_id exactly. Once a first revision didn't fully
-- resolve an objection and nothing auto-raised a fresh conflict against the
-- revision itself, the original conflict -- now attached to a superseded
-- ancestor proposal -- could never be cited again: there was no WebMCP path
-- to submit a second revision addressing it, contradicting
-- docs/webmcp-demo.md's explicit guidance to "ask it to try again."
--
-- Fix: allow a conflict anywhere in the active proposal's own lineage
-- (walking parent_proposal_id upward, the same recursive pattern already
-- used in build_final_decision_candidate), not only the exact active
-- proposal. parent_proposal_id is set once at creation and never mutated, so
-- this can only ever reach the active proposal's own ancestor chain -- never
-- a sibling branch or an unrelated proposal elsewhere in the room. Only
-- reachability through revision history is relaxed; room-wide access is
-- unchanged.
...
begin
  ...
  where conflict_value.id = any(p_conflict_ids)
    and conflict_value.room_id = p_room_id
    and conflict_value.status = 'open'
    and conflict_value.proposal_id in (
      with recursive lineage as (
        select proposal.id, proposal.parent_proposal_id
        from public.proposals proposal
        where proposal.id = source_proposal_id and proposal.room_id = p_room_id
        union all
  ...
  if valid_conflict_count <> supplied_conflict_count then
    return public.action_failure(
      'VALIDATION_ERROR',
      'Every conflict must be open, belong to this room, and target the active proposal or one of its ancestors.',
      current_version
    );
  end if;
